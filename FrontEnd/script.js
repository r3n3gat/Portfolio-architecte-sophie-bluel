...
function filterWorksByCategory(categoryId) {
  if (categoryId === "") {
    // Attribut vide du html pour le bouton "Tous"
...
async function initProject() {
  try {
    ...
    // Récupération des categories et création dynamique des filtres
    ...
    allButton.setAttribute("data-category-id", "");
    allButton.addEventListener("click", function () {
      filterWorksByCategory(this.getAttribute("data-category-id"));
    });
    filterContainer.appendChild(allButton);

    // Création des boutons à partir de la recup de categories
    for (let category of categories) {
      const filterButton = document.createElement("button");
      filterButton.textContent = category.name;
      filterButton.setAttribute("data-category-id", category.id.toString());
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (photoPreviewElement.firstChild) {
      photoPreviewElement.removeChild(photoPreviewElement.firstChild);
    }
    const elementsHidden = document.querySelectorAll(
      ".modal p, .modal i.fa-image"
    );
